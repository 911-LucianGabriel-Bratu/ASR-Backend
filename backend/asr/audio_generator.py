...
from .utils import calc_feat_dim, spectrogram_from_file, text_to_int_sequence, shuffle_data, sort_data, \
    spectrogram_using_audio_file
...
class AudioGenerator:
    ...
    def load_train_data(self, desc_file=os.path.join(settings.BASE_DIR, 'backend', 'asr', 'train.json')):
        self.load_metadata_from_desc_file(desc_file, 'train')
        self.fit_train()
        if self.sort_by_duration:
            self.sort_data_by_duration('train')

    def load_validation_data(self, desc_file=os.path.join(settings.BASE_DIR, 'backend', 'asr', 'valid.json')):
        self.load_metadata_from_desc_file(desc_file, 'validation')
        if self.sort_by_duration:
            self.sort_data_by_duration('valid')

    def load_test_data(self, desc_file=os.path.join(settings.BASE_DIR, 'backend', 'asr', 'test.json')):
    ...
    def load_metadata_from_desc_file(self, desc_file, partition):
        ...
        if not entered_loop:
            print("did not even open the file")
        if partition == 'train':
    ...
    def featurize_audio_file(self, wav_file):
        audio_data, sample_rate = soundfile.read(wav_file, dtype='float32')
        return spectrogram_using_audio_file(
            audio_data, sample_rate=sample_rate, step=self.step, window=self.window,
            max_freq=self.max_freq)

    def get_sample_rate_from_wav_data(self, wav_data):
